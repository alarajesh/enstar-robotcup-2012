/* Ce fichier contient le programme commandant les pinces du robot. L'adresse de la carte moteur est 0x58.
 *  Deux fonctions sont à utiliser : OuvrirPince() (ouvre les pinces) et FermerPince() (ferme les pinces). 
 *  L'amplitude de l'ouverture fermeture se règle dans le code de ces fonctions : changer la valeur seuil de getEncoder1() et getEncoder2().               
 *  Lorque les encodeurs1 et 2 sont à zéro, cela correspond à la position ouverte des pinces   */
...
/* Lors de la fermeture des pinces, le moteur tourne dans le sens horaire. Quand la pince est en position ouverte, l'encodeur est à 0. Donc tant que
 l'encodeur est en dessous d'une certaine valeur (50 par exemple), on ordonne au moteur de tourner dans le sens antihoraire
 */
void FermerPince(){

  while(getEncoder1() <= 50 || getEncoder2() <= 50 ){// Faire tourner le(s) moteur(s) qui n'ont pas encore atteint la valeur seuil
    ...
    if(getEncoder1() <= 50){// Tourner moteur1 tant que encodeur 1 est en dessous de la valeur seuil
      setSpeed1(140);
    }
    else if(getEncoder1() > 50){// Arrêter le moteur 1 si la valeur seuil est atteinte
      setSpeed1(128);
    }
    if(getEncoder2() <= 50){// Tourner moteur2 tant que encodeur 2 est en dessous de la valeur seuil
      setSpeed2(140);
    }
    else if(getEncoder1() > 50){// Arrêter le moteur 2 si la valeur seuil est atteinte
      setSpeed2(128);
    }
...
void OuvrirPince(){
  ...
  while(getEncoder1() >= 0 || getEncoder2() >=0  ){// Faire tourner le(s) moteur(s) qui n'ont pas encore atteint la valeur seuil
    ...
    if(getEncoder1() >= 0){// L'encodeur 1 a une valeur >=0, on fait tourner le moteur1 dans le sens antihoraire
      setSpeed1(116);
    }
    else if(getEncoder1() < 0){// La valeur de l'encodeur1 est <0, arrêter moteur1 car la pince est en position ouverte
      setSpeed1(128);
    }
    if(getEncoder2() >= 0){// L'encodeur 2 a une valeur >=0, on fait tourner le moteur2 dans le sens antihoraire
      setSpeed2(116);
    }
    else if(getEncoder2() < 0){// La valeur de l'encodeur2 est <0, arrêter moteur2 car la pince est en position ouverte
      setSpeed2(128);
    }
  }
...
void rotate(int x){// Fait tourner le robot : x=458 <--> 90°

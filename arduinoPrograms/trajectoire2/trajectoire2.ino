...
unsigned long sampling_period_rotation = 200; // Periode d'échantillonage (pour asservissement en rotation) en millisecondes  //!\\ Si elle trop grande ou petite, l'asservissement devient instable
...
bool flag = false;// Flag levé lorsque l'arrêt d'urgence est enclenché(cf. attachInterrupt dans le setup() )
...
void setup()
{
  ...
  attachInterrupt(0, arretUrgence, CHANGE);// Appel à la fonction arretUrgence quand l'interruption est déclenchée
  Serial.println("après");
  //Initialisation des encodeurs
  resetEncoders();
  delay(2000);// Attente de 2 secondes
  setAccelerationRate(1);// Réglage de l'accélération max (1 : mini; max : maxi)
  getAccelerationRate();
...
void asservissement_distance(long x)
{
  ...
  commande_translation = Kp_translation*(x - distance_current) + Kd_translation*(distance_old - distance_current);
  ...
  {
    setSpeed1(128 + commande_translation);
    setSpeed2(128 + commande_translation);
  }
...
void asservissement_rotation(int x){// Fait tourner le robot : x=458 <--> 90°
  ...
  commande_rotation =  Kp_rotation*(x - orientation_current) + Kd_rotation*(orientation_old - orientation_current) ; // J'ai changé le signe!!
  Serial.print("Commande : ");
  Serial.println(commande_rotation);
  Serial.println(" ");
  
  if (commande_rotation <= 127 && commande_rotation >= -127 ){
    setSpeed1(commande_rotation + 128);
    setSpeed2(-commande_rotation + 128);
  }
  else if (commande_rotation >= 128){ // Saturation de la vitesse
    setSpeed1(255);
    setSpeed2(0);
  }
  else if(commande_rotation <= -128){ // Saturation de la vitesse
...
// Fonction appelée lorque l'arrêt d'urgence est enclenché
void arretUrgence(){
  flag = true;  
}
